...
from util.viz import chain_viz
...
def create_scene_hypotheses_sequential(sound_name, sound_group, expt_name, experiment_config, segmentation, round_idx):
    ...
    # 1. Create event proposals and observation. If there aren't any left, inference is done!
    ...
    if round_idx == 0: os.makedirs(hypotheses_dir, exist_ok=True)
    keep_idxs = [] if round_idx == 0 else analyze_previous_round(hypotheses_dir, round_idx)

    # 3. Propose hypotheses for current round
    scene_hypotheses = create_new_scene_hypotheses(event_proposal_sequence, keep_idxs, round_idx, experiment_config, observation, audio_sr, hypotheses_dir)
    
    # 3. Filter hypotheses using sequential_heuristics
    if experiment_config["heuristics"]["sequential"]["use"] == True:
        scene_hypotheses = inference.sequential_heuristics.prioritize(scene_hypotheses, experiment_config, round_idx=round_idx)
    # 3b. Add background only on round_idx == 0 if desired - might double the number of proposals
    if experiment_config["optimization"]["add_background_noise_proposal"] and round_idx == 0:
        ...
        inference.proposals.add_background_proposal(scene_hypotheses, round_idx, experiment_config, observation, audio_sr, full_scene_duration)
...
def iterative(sound_name, sound_group, expt_name, config_name, segmentation, round_type, round_idx=0, seed=None):
    """ Run inference for a scene using amortized event proposals by submitting each hypothesis as a new slurm job """
    ...
    if "sequential" in round_type:
        scene_hypotheses, last_round_of_sequential = create_scene_hypotheses_sequential(sound_name, sound_group, expt_name, experiment_config, segmentation, round_idx)
        if (scene_hypotheses is None) and (last_round_of_sequential is None):
            #Inference is done.
            slurm_launch(on_complete, cpu=True, job_name=f"complete", log_dir=f"{hypotheses_dir}/slurm_logs")()
            return
        if last_round_of_sequential:
            round_type = round_type + "_final"
    elif "cleanup" in round_type:
        scene_hypotheses, original_savepaths = create_scene_hypotheses_cleanup(sound_name, sound_group, expt_name, experiment_config, round_type, round_idx)

    # 4. Create folders for all the hypotheses
    for hypothesis_number, scene_hypothesis in enumerate(scene_hypotheses):
        hypothesis_name = get_hypothesis_name(round_idx, hypothesis_number, round_type)
        savepath = inference.io.get_directory(sound_name, sound_group, expt_name, hypothesis_name=hypothesis_name, seed=None)
        if "cleanup" in round_type and hypothesis_number < experiment_config["optimization"]["n_to_keep"]:
            ...
            if not experiment_config["heuristics"]["cleanup"]["optimize_originals"]:
                ...
                # Note: if you ever want to optimize originals, removing state.txt may be necessary
    ...
    job_array_id = slurm_launch(solo_optimization, array=range(len(scene_hypotheses)), job_name=f"R{round_idx}", log_dir=f"{hypotheses_dir}/slurm_logs", return_jobid=True)(
        earlystop = earlystop, round_type=round_type)

    # 6. if early_stop, figure out which ones to continue. otherwise, analyze results of this round.
    if earlystop:
        slurm_launch(continue_after_early_stop, cpu=True, dependency=f"afterany:{job_array_id}", job_name=f"R{round_idx}continue", log_dir=f"{hypotheses_dir}/slurm_logs")(
            round_idx=round_idx, round_type=round_type)
    else:
        # analyze_complete_round will also launch viz and next iteration=round_idx+1
        slurm_launch(analyze_complete_round, cpu=True, dependency=f"afterany:{job_array_id}", job_name=f"R{round_idx}complete", log_dir=f"{hypotheses_dir}/slurm_logs")(
            round_idx=round_idx, round_type=round_type)
...
def trim_observation(full_observation, audio_sr, config, scene_offset=-1):
    ...
    if scene_offset > 0:
        ...
        off_ramp = renderer.util.hann_ramp(audio_sr, ramp_duration = config["renderer"]["ramp_duration"])
...
def get_event_proposals_and_observation(round_idx, sound_name, sound_group, segmentation, experiment_config):
    ...
    event_proposals, full_observation, audio_sr = inference.proposals.load_event_proposals_and_sound(sound_name, sound_group, amortize, source_types_to_keep=experiment_config["hyperpriors"]["source_type"]["args"], minimum_duration=experiment_config["renderer"]["steps"]["t"])
    full_scene_duration = len(full_observation)/audio_sr

    # Group event proposals based on their temporal sequence 
    event_proposal_sequence = inference.proposals.group_event_proposals(event_proposals, experiment_config)
    ...
    new_event_proposals = [(es["events"][0]["meta"]["rank"], es) for es in event_proposal_sequence if es["events"][0]["meta"]["round_idx"] == round_idx]

    # If round_idx is the max of the round_idxs, then we are on the last round and we will look at the entire observation
    last_round = np.max([es["events"][0]["meta"]["round_idx"] for es in event_proposal_sequence])
    ...
    observation = trim_observation(full_observation, audio_sr, experiment_config, scene_offset=scene_offset)
...
def continue_after_early_stop(sound_name, sound_group, expt_name, config_name, round_type, round_idx):
    ...
    previous_hypothesis_folders = glob(hypotheses_dir + get_hypothesis_name(round_idx, "*", round_type) + "/")
    last_round_elbos = {}
    failed_hypotheses = []
    ...
    if len(failed_hypotheses) > 0:
        # Need to resubmit those earlystop that crashed because of slurm, then recheck which won
        print("slurm-Failed hypotheses (resubmitting): ", failed_hypotheses)
        job_array_id = slurm_launch(solo_optimization, array=failed_hypotheses, job_name=f"R{round_idx}", log_dir=f"{hypotheses_dir}/slurm_logs", return_jobid=True)(
            earlystop = True, round_type=round_type)
        slurm_launch(continue_after_early_stop, cpu=True, dependency=f"afterany:{job_array_id}", job_name=f"R{round_idx}continue", log_dir=f"{hypotheses_dir}/slurm_logs")(
            round_idx=round_idx, round_type=round_type)
    else:
        # 2. Figure out what did best, filter out the shit ones
        ordered_hypotheses = sorted(last_round_elbos.keys(), key=lambda k: last_round_elbos[k], reverse=True)
        ordered_hypotheses = [oh for oh in ordered_hypotheses if not np.isinf(last_round_elbos[oh])]
        ...
        job_array_id = slurm_launch(solo_optimization, array=keep_idxs, return_jobid=True, job_name=f"R{round_idx}continue", log_dir=f"{hypotheses_dir}/slurm_logs")()

        # 4. SBATCH --dependency=after complete(round_idx)
        slurm_launch(analyze_complete_round, cpu=True, dependency=f"afterany:{job_array_id}", job_name=f"R{round_idx}complete", log_dir=f"{hypotheses_dir}/slurm_logs")(round_idx=round_idx)


def analyze_complete_round(sound_name, sound_group, expt_name, config_name, round_type, round_idx):
    """ Analyze complete round=round_idx of iterative inference to pass on the top N hypotheses to round=round_idx+1"""
    ...
    hypotheses_dir = os.path.join(os.environ["inference_dir"], expt_name, sound_group, sound_name, "")
    previous_hypothesis_folders = glob(hypotheses_dir + get_hypothesis_name(round_idx, "*", round_type) + "/")    
    last_round_elbos = {}; failed_hypotheses = [];
    for ph_folder in previous_hypothesis_folders:
        hypothesis_number = int(re.match(".*/round[0-9]*-([0-9]*)", ph_folder).group(1))
        with open(ph_folder + "state.txt", "r") as f:
            hypothesis_status = f.readlines()
            print(f"{hypothesis_number}: {hypothesis_status}")
        if "".join(hypothesis_status) == "continue": #other options: earlystop, complete
            failed_hypotheses.append(hypothesis_number)
        try:
            with open(ph_folder + "metrics.json", "r") as f:
                metrics = json.load(f)
    ...
    if len(failed_hypotheses) > 0:
        print("slurm-Failed hypotheses (resubmitting): ", failed_hypotheses)
        # If slurm failed on any hypotheses, resubmit those hypotheses and then redo analyze_complete_round
        job_array_id = slurm_launch(solo_optimization, array=failed_hypotheses, job_name=f"R{round_idx}", log_dir=f"{hypotheses_dir}/slurm_logs", return_jobid=True)(
        ...
        print(f"On round {round_idx}, {round_type}: keep=", keep_idxs, ", reject=", reject_idxs, flush=True)
        ...
        if os.path.isfile(results_fn):
            ...
            if n_rounds_in_saved_results == round_idx:
                ...
                print(f"results.json already has correct indices for round {round_idx}. Skipping save.")
            else:
        ...
        # Save round type
        round_type_fn = hypotheses_dir + "roundtype.txt"
        round_type_f = open(round_type_fn, "a")
        round_type_f.write(f"{round_idx}:{round_type}\n")
        round_type_f.close()

        # Launch viz for this complete round
        slurm_launch(viz, cpu=True, job_name=f"R{round_idx}viz", log_dir=f"{hypotheses_dir}/slurm_logs")()

        # Launch next round
        if ("cleanup" in round_type) or (round_type == "sequential_final"):
            round_type = inference.cleanup.get_next_round_type(round_type, keep_idxs=keep_idxs, use_cleanup_flag=experiment_config["heuristics"]["cleanup"].get("use", True))
            if round_type is None:
                print("Inference done, we made it! Exiting.")
                slurm_launch(on_complete, cpu=True, job_name=f"complete", log_dir=f"{hypotheses_dir}/slurm_logs")()
                return

        slurm_launch(iterative, cpu=True, mem=100, job_name=f"R{round_idx+1}", log_dir=f"{hypotheses_dir}/slurm_logs")(round_type=round_type, round_idx=round_idx+1)
...
def solo_optimization(sound_name, sound_group, expt_name, round_type, round_idx, hypothesis_number, robust, earlystop=False, seed=None, cuda_idx=0):
    """Infer variational posterior for a hypothesis which already has an initialization saved in a folder"""
    manual_seed(seed)
    ...
    scene_opt, optimizers, schedulers, metrics, hypothesis_config, savepath = hypothesis_setup(sound_name, sound_group, expt_name, hypothesis_name, None, None, None, device)
    with context(hypothesis_config, batch_size=hypothesis_config["optimization"]["batch_size"]):
        elbo_is = inference.optimize.basic_loop_from_scene(scene_opt, optimizers, schedulers, metrics, hypothesis_config, savepath=savepath, earlystop=earlystop, round_type=round_type, accumulate_gradients=robust)
...
def save_solo_optimization(sound_name, sound_group, expt_name, round_type, round_idx, hypothesis_number, robust, earlystop=False, seed=None, cuda_idx=0):
    """ Save a file which proves that slurm did not crash; rather, optimization finished or it crashed because of cuda"""
    hypothesis_name = get_hypothesis_name(round_idx, hypothesis_number)
    savepath = inference.io.get_directory(sound_name, sound_group, expt_name, hypothesis_name=hypothesis_name, seed=None)
...
def slurm_launch(f, cpu=None, array=None, n_mins=150, job_name=None, log_dir=None, dependency=None, mem=32, return_jobid=False, confirm=False):
    ...
    def wrapped_f(**kwargs):
        ...
        sbatch_args['exclude'] = get_exclude_nodes(sbatch_args.get('gres', None))
...
def _launch_multiple(*sound_names):
    ...
    slurm_launch(iterative)(sound_name=sound_name, sound_queue=sound_queue, round_idx=0, round_type="sequential")
...
def sweep(sound_group, sound_queue):
    if sound_queue is None:
        ...
        _launch_multiple(*[os.path.splitext(os.path.basename(w))[0] for w in wavs])
    else:
        _launch_multiple(*sound_queue.split(","))
...
if __name__ == "__main__":
    ...
    # Experiment args
    parser.add_argument("--sound_name", type=str, default=None)
    ...
    parser.add_argument("--round_type", type=str, default="sequential")
    ...
    # Queue
    parser.add_argument("--sound_queue", type=str, default=None)
    ...
    f_args = {k:v for k, v in PY_ARGS.items() if k in inspect.getfullargspec(f)[0]}
